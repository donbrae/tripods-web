...
TRIPODS.mvt = (function (_module) {
    ...
    _this.getMeasurements = function () {
        if (!isNaN(_module.game_state.level)) {
            ...
            // WebKit having performance issues rendering shadow (along with blur) on jump, so commented out for now. To restore, add below variables to `filter` property in relevant [keyframe] arrays
        ...
        const cb = fade_in ? () => {
            pivot_timeout = setTimeout(() => {
                if (_module.game_state.level_running) {
                    _module.utils.fadeIn("#pivotor", 180);
                }
            }, delay);
        } : null;
